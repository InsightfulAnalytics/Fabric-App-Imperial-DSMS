...
- `components/chrome/` — `Panel`, `SectionHeader`, `SidebarRail`, `TopBar`

**UI kit** (`ui_kits/fabric-console/`) — the four-screen interactive console. See its `README.md`.

**Assets** (`assets/`) — `dsms-seal.svg`, `dsms-wordmark.svg`.

**Starting points** — `Button` (Forms), `KpiTile` (Data), `Panel` (Chrome), `DataTable` (Data), and the full **Fabric Console** screen.
