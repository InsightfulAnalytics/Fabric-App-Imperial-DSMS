...
## Accepting a Claude Design Handoff

When the user says "I've handed off from Claude Design" or "process the design import in design-imports/<name>/", follow these steps in order:

### 1. Read the import directory

```
design-imports/<name>/
  manifest.json       ← parsed visual requirements (run import-design if missing)
  *.html              ← Claude Design export HTML (inspect for layout context)
  images/             ← screenshots of the designed visuals
```

If `manifest.json` is missing, run:
```bash
npm run import-design -- --zip <path-to-zip> --name <name>
```

### 2. Generate mockup specs

For each visual in `manifest.visuals`, create a `.vl.json5` spec in `mockups/` using:
- The matching template from `design-system/spec-templates/` as a starting point
- The `fields` array from the manifest as the DATA BINDING block
- Field `dax_hint` values as the DAX column/measure references
- Sample data (inline `values` array) to verify the spec renders correctly

Visual type → template mapping:
| Manifest `type`  | Template |
|-----------------|----------|
| `bullet`        | `bullet-chart.vl.json5` |
| `bar`           | `bar-chart.vl.json5` |
| `sparkline`     | `sparkline.vl.json5` |
| `kpi-card`      | `kpi-card.vl.json5` |
| `line`          | `line-chart.vl.json5` |

Name each mockup file after the manifest visual `id` (e.g., `mockups/bullet-chart.vl.json5`).

### 3. Preview specs in Vega Viewer

Run the mockup watcher, then open each `.preview/*.vl.json` file in Vega Viewer to confirm visuals render as expected before wiring production files:

```bash
npm run mockups:watch
```

Fix any spec errors before proceeding.

### 4. Scaffold production files

For each approved mockup spec, run `wire-spec` to generate the `.json`, `.ts`, and `.dax` files:

```bash
npm run wire-spec -- mockups/<id>.vl.json5 --domain <section> --name <id>
```

Use the manifest visual's `section` (or the page name from `data-section`) as `--domain`.

### 5. Write DAX queries

For each generated `.dax` file in `src/queries/`:
- Use the field `dax_hint` values from the manifest as starting points for column/measure references
- Follow the [dax-authoring](.agents/skills/dax-authoring/SKILL.md) skill
- Test with: `npx fabric-app-data query <alias> --file src/queries/<domain>/<name>.dax --limit 5`
- Copy exact column names from CLI output → update `columnMetadata` keys in the `.ts` factory file

### 6. Build the React page component

Build the page component following the [app-design](.agents/skills/app-design/SKILL.md) skill. Use the HTML layout from the import directory as a guide for positioning visuals. Import factory functions from `src/queries/` and use `useSemanticModelQuery` + `toDataTable` + `VegaVisual` as shown in the [Build components](#5-build-components-with-data-app-code-phase) section above.
